        ...
        match cmd {
						...
						if play {
                            match self.state {
                                ...
                                | PlayerState::EndOfTrack {
									...
									track_name: audio.name.to_string(),	//1
                                }),
                                _ => self.send_event(PlayerEvent::Started { track_id: track_id, track_name: audio.name.to_string(), }),	//2
                            }
                            ...
                            match self.state {
                                ...
                                | PlayerState::EndOfTrack {
                                    ...
                                    old_track_id: old_track_id,
                                    new_track_id: track_id,
									track_name: audio.name.to_string(),	//3
                                }),
                                _ => (),
                            }
                            self.send_event(PlayerEvent::Stopped { track_id: track_id, track_name: audio.name.to_string(), });	//4
                        }
                    }
            ...
            PlayerCommand::Play => {
                if let PlayerState::Paused { track_id, .. } = self.state {
                    ...
                    self.send_event(PlayerEvent::Started { track_id: track_id, track_name: "(1)".to_string(), });
                    self.start_sink();
                } else {
            ...
            PlayerCommand::Pause => {
                if let PlayerState::Playing { track_id, .. } = self.state {
                    ...
                    self.stop_sink_if_running();
                    self.send_event(PlayerEvent::Stopped { track_id: track_id, track_name: "(2)".to_string(), });
                } else {
            ...
            PlayerCommand::Stop => match self.state {
                ...
                | PlayerState::EndOfTrack { track_id } => {
                    self.stop_sink_if_running();
                    self.send_event(PlayerEvent::Stopped { track_id: track_id, track_name: "(3)".to_string(), });
